...
function validateForm() {
    ...
    var x = document.getElementById("fw").value;
    var y = document.getElementById("sw").value;
    var len=document.getElementById("len").value;
    var letters = /^[A-Za-z]+$/

    if (x == null || x == "" || y == null || y == "") {
        alert("We need two words!");
        return false;
    }
    ...
    else if(len<=0||len>x.length+y.length){
        ...
        if (isVowel(y.charAt(i)) == true) { //isVowel returns a boolean saying if a character is a vowel
            ...
            //if y.charAt(i) is also in x:
                if ((x.length-closestX + i) < difference) {
                    ...
                    port = port.charAt(0).toUpperCase() + port.slice(1,len);
                    }
                }
            }
        }
